...
export const manageMemoryTool: Tool = {
  ...
  handler: (args: unknown): string => {
    ...
    // Dynamic import not possible in sync handler — import at module level
    const { useMemoryStore } = require('@/stores/memoryStore') as {
      useMemoryStore: typeof import('@/stores/memoryStore').useMemoryStore;
    };

...
The Excel Desktop suite includes four always-on live Copilot AI round-trip checks. Run `npm run dev` before `npm run test:e2e`; a missing proxy is a test failure, not a skip.
...
`npm test` must continue to include the live Copilot integration specs by default. Do not add env-gated opt-ins or skip paths for Copilot/live AI coverage; if the proxy is unavailable, start `npm run dev`, and if the tests still fail, treat that as a blocker.
